...
class LiveTapInService : Service() {

    companion object {
        const val TAG = "LiveTapInService"
        const val CHANNEL_ID = "live_tapin_channel"
    ...
    private val tapInStates = mutableMapOf<Int, TapInState>()
    private val serviceScope = CoroutineScope(Dispatchers.IO + SupervisorJob())
    private var apiServiceForTapIn: ApiServices? = null
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (apiServiceForTapIn == null && config != null) {
            apiServiceForTapIn = createApiServiceForTapIn(config)
        }

        when (intent?.action) {
            ACTION_START -> startTapIn(intent, notificationId)
            ACTION_STOP -> stopTapIn(notificationId, fromNotification)
            ACTION_DISMISS -> dismissTapIn(notificationId)
        }
    ...
    private fun createChannelForNotification(notificationId: Int, title: String) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            val channel = NotificationChannel(
                ...
                description = "Tap In notification for $title"
                setShowBadge(false)
    ...
    private fun startTapIn(intent: Intent, notificationId: Int) {
        if (tapInStates.containsKey(notificationId)) {
            return
        }

        val title = intent.getStringExtra(EXTRA_TITLE) ?: "Tap In"
        ...
        val runnable = object : Runnable {
            override fun run() {
                tapInStates[notificationId]?.let {
        ...
        tapInStates[notificationId] = TapInState(
    ...
    private fun stopTapIn(id: Int, fromNotification: Boolean) {
        tapInStates[id]?.let {
            handler.removeCallbacks(it.runnable)
            tapInStates.remove(id)
            
            if (tapInStates.isEmpty()) {
                stopForeground(true)
            }
        ...
        if (tapInStates.isEmpty()) stopSelf()
    }

    private fun dismissTapIn(id: Int) {
        tapInStates[id]?.apply {
            isDismissed = true
        }
    }

    private fun updateNotification(id: Int) {
        val state = tapInStates[id] ?: return
    ...
    private fun buildNotification(id: Int): Notification {
        val state = tapInStates[id] ?: return NotificationCompat.Builder(
            this,
        ...
        val tapInIcon = if (isDark) R.drawable.ic_hand_tap_fill else R.drawable.ic_hand_tap_fill_black // Fallback icon
        val stopIcon =  if (isDark) R.drawable.ic_stop else R.drawable.ic_stop_black

        rv.setImageViewResource(R.id.ivView, tapInIcon)
        ...
        // Only one action for tapIn: Stop
    ...
    private fun createActionIntent(id: Int, action: String): PendingIntent =
        PendingIntent.getService(
            this,
            id + action.hashCode(),
            Intent(this, LiveTapInService::class.java).apply {
                this.action = action
    ...
    private fun createApiServiceForTapIn(config: LiveUpdateConfig): ApiServices? {
        val endpoint = config.apiEndpoint?.tapInEndpoints ?: return null
    ...
    private fun postTapInStop(id: Int) {
        val api = apiServiceForTapIn ?: return
        serviceScope.launch {
            try {
                api.postTapInStop(id)
            } catch (e: Exception) {
                Log.e(TAG, "API Error: postTapInStop failed", e)
            }
        }
    }
